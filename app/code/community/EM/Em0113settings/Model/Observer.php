<?php
class EM_Em0113settings_Model_Observer {
	
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'em0113')){	
			# Disable default magento navigation
			if (Mage::helper('em0113settings')->getGeneral_DisableDefaultNav()&&(Mage::getConfig()->getModuleConfig('EM_Megamenupro')->is('active', 'true'))) {
			...
			if (Mage::helper('em0113settings')->getGeneral_DisableFrontendVariation()) {
			...
			if (Mage::helper('em0113settings')->getGeneral_DisableFooterLinks()) {
	...
	public function beforeCatalogProductCollectionLoad(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'em0113')){		
			$alt_img = Mage::helper('em0113settings')->getProductsGrid_AltImg();
			if ($alt_img == 'image')
