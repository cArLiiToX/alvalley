;(function ($) {
	...
	$.csslider = function(el,options) {
		...
		slider.adaptArrays = [[0,760],[760,980],[980,1200],[1200,1600],[1600]];
		slider.setup = function(){
			var i = slider.getNewCurrentAdapt();
			if(i != slider.currentAdapt)
				slider.currentAdapt = i;
		...
		slider.getNewCurrentAdapt = function(){
			var width = $(window).width();
			var i = 0;
			for(i;i < slider.adaptArrays.length-1;i++){
				var tmp = slider.adaptArrays[i];
				if(tmp.length > 1 && width <= tmp[1] && width > tmp[0]){
					break;
				}
			}
			return i;
		...
		slider.init = function (first){
			...
			if(!vertical){ /* horizontal */
				...
				if((first || slider.calHeight) && vars.recalHeight){
					//setTimeout(function(){
